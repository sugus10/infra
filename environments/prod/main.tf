...
locals {
  ...
  azs      = slice(data.aws_availability_zones.available.names, 0, 3) # All 3 AZs for production
...
module "vpc" {
  ...
  single_nat_gateway = var.single_nat_gateway # Configurable for production
  tags               = local.tags
}
  ...
  budget_notifications = [
    {
      ...
      threshold                  = 70 # Earlier alert for production
    ...
    {
      comparison_operator        = "GREATER_THAN"
      threshold                  = 90 # Earlier alert for production
      threshold_type             = "PERCENTAGE"
      notification_type          = "ACTUAL"
      subscriber_email_addresses = [var.budget_alert_email]
    },
